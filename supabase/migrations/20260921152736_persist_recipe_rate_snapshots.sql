-- Preserve recipe-specific pricing. NULL keeps legacy recipes on their existing master fallback.
ALTER TABLE public.recipe_items
  ADD COLUMN IF NOT EXISTS rate NUMERIC(14,4) CHECK (rate >= 0 AND rate::TEXT NOT IN ('NaN', 'Infinity', '-Infinity')),
...
BEGIN
  ...
  IF p_items IS NULL OR jsonb_typeof(p_items) IS DISTINCT FROM 'array' THEN
    RAISE EXCEPTION 'Recipe items must be an array' USING ERRCODE = '22023';
  END IF;
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    v_rate := COALESCE((v_item->>'rate')::NUMERIC, v_ing.current_rate, 0);
    v_eff_rate_unit := COALESCE(NULLIF(TRIM(v_item->>'rate_unit'), ''), v_ing.rate_unit, v_ing.base_unit, v_unit);
    IF v_rate < 0 OR v_rate::TEXT IN ('NaN', 'Infinity', '-Infinity') THEN
      RAISE EXCEPTION 'Ingredient rate must be a finite nonnegative number' USING ERRCODE = '22023';
    END IF;
    IF NOT (v_unit = v_eff_rate_unit
      OR (v_unit IN ('g', 'kg') AND v_eff_rate_unit IN ('g', 'kg'))
      OR (v_unit IN ('ml', 'litre') AND v_eff_rate_unit IN ('ml', 'litre'))) THEN
      RAISE EXCEPTION 'Incompatible quantity and rate units: % / %', v_unit, v_eff_rate_unit USING ERRCODE = '22023';
    END IF;
...
REVOKE ALL ON FUNCTION public.save_recipe_version_transaction(UUID, UUID, TEXT, INTEGER, JSONB, JSONB, TEXT, TEXT, UUID) FROM PUBLIC, anon;
