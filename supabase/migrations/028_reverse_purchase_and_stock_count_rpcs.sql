...
BEGIN
  ...
  FOR v_item IN SELECT * FROM material_purchase_items WHERE purchase_id = p_purchase_id LOOP
    ...
    INSERT INTO raw_material_movements (
      ...
      'purchase_reversal',
      -ABS(v_qty),
      COALESCE(v_item.purchase_unit, (SELECT base_unit FROM ingredients WHERE id = v_item.ingredient_id), 'unit'),
  ...
  FOR v_item IN SELECT * FROM physical_stock_count_items WHERE physical_stock_count_id = p_count_id LOOP
    ...
    IF v_discrepancy <> 0 THEN
      INSERT INTO raw_material_movements (
        ...
        v_discrepancy,
        COALESCE((SELECT base_unit FROM ingredients WHERE id = v_item.ingredient_id), 'kg'),
        COALESCE((SELECT current_rate FROM ingredients WHERE id = v_item.ingredient_id), 0.00),
        0.00,
