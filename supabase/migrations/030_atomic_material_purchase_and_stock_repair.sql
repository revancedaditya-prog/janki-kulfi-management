-- ============================================================================
-- Migration 030: Atomic Material Purchase RPC, Authoritative Stock View, & Idempotent Repair
-- ============================================================================
...
-- 2. Atomic Material Purchase RPC Function
...
BEGIN
  -- Set strict search path for security
  ...
  -- 2. Resolve User ID (from auth.uid() or parameter)
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- B. Insert Authoritative Stock-In Movement
...
-- 3. Robust Material Purchase Reversal RPC Function
...
-- 4. Idempotent Backfill & Reconciliation for PUR-20260909-4978 (and any orphaned received purchases)
DO $$
DECLARE
  r_purch RECORD;
  r_item RECORD;
  ...
  v_count INTEGER := 0;
BEGIN
  -- Search for received material purchases that lack movements
  FOR r_purch IN 
  ...
  LOOP
    ...
    LOOP
      ...
      -- Check if this item is already recorded in raw_material_movements
      SELECT EXISTS (
        SELECT 1 FROM public.raw_material_movements
        WHERE reference_table = 'material_purchases'
          AND reference_id = r_purch.id
          AND ingredient_id = v_ing_uuid
      ) INTO v_mov_exists;

      IF NOT v_mov_exists THEN
        ...
        IF v_qty > 0 THEN
          INSERT INTO public.raw_material_movements (
            ...
            'Supplier',
            'Main Store',
            'Reconciled / Backfilled missing stock movement for purchase ' || r_purch.purchase_number,
            r_purch.created_by
          );

          -- Update ingredient current_rate if zero
          ...
          INSERT INTO public.audit_logs (
            ...
            jsonb_build_object(
              ...
              'ingredient_id', v_ing_uuid,
              'reconciled_quantity', v_qty
            ),
            'Idempotent backfill of missing purchase stock movement',
            r_purch.created_by
          );
  ...
  RAISE NOTICE 'Reconciled % orphaned purchase stock movement(s).', v_count;
END $$;

-- 5. Grant Permissions and Notify Schema Reload
